...
const Auth = () => {
  ...
  const handleSignUp = (e) => {
    ...
    if(!email.length && !password.length && !username.length ){
      setNotif({ message: "Please fill the details", type: "error" });
      return;
    }
    else if(!email.length ){
      setNotif({ message: "Email is required", type: "error" });
      return;
    }
    else if(!password.length ){
      setNotif({ message: "Password is required", type: "error" });
      return;
    }
    else if(!username.length ){
      setNotif({ message: "Username is required", type: "error" });
      return;
    }
    else if(password.length<7){
      setNotif({message: "Password should be greater than 6 characters", type: "error"});
      return;
    }
    else if(password !== confirmPassword){
      setNotif({message: "Confirm password must match the password.", type: "error"});
      return;
    }
    else{
      console.log("submitted");
      setNotif({message: "user created",type:"success"});
    }
